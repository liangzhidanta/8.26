...
from model import LowRankProjSymmetric, ResidualMLP, get_model_by_name
from rank_allocator import RankAllocatorWithUpdate
...
def orth_regularizer(A: torch.Tensor, B: torch.Tensor, eps: float = 1e-6) -> torch.Tensor:
    """激活级正交正则：R(P,Q)=||P^T P - I||_F^2 + ||Q Q^T - I||_F^2
    A: (B,T,r), B: (B,r,D)
    """
    Bsz, T, r = A.shape
    _, r2, D = B.shape
    assert r == r2
    I_r = torch.eye(r, device=A.device, dtype=A.dtype)
    reg = 0.0
    for b in range(Bsz):
        P = A[b]  # (T,r)
        Q = B[b]  # (r,D)
        PtP = (P.transpose(0, 1) @ P) / (T + eps)  # (r,r)
        QQt = (Q @ Q.transpose(0, 1)) / (D + eps)  # (r,r)
        reg = reg + torch.norm(PtP - I_r, p='fro')**2 + torch.norm(QQt - I_r, p='fro')**2
    return reg / Bsz


def train_one_epoch(model, dataloader, optimizer, device, lambda_B, lambda_ortho, lambda_act=0.0):
    """训练一个epoch"""
    model.train()
    total_loss = 0.0
    total_mse = 0.0
    ...
    for (X,) in pbar:
        ...
        loss = mse
        if lambda_act and lambda_act > 0:
            loss = loss + lambda_act * orth_regularizer(A, B)
...
def plot_curves(history, results_dir):
    ...
    plt.plot(epochs, val_mses, 'r-', linewidth=1.5, label='Val MSE')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    ...
    # 第二个子图：验证MSE（保持原样）
    plt.subplot(1, 3, 2)
    plt.plot(epochs, val_mses, 'r-', linewidth=2, label='Val MSE')
    plt.xlabel('Epoch')
    plt.ylabel('MSE')
    ...
    plt.subplot(1, 3, 3)
    plt.plot(epochs, rel_f_scores, 'g-', linewidth=2, label='Rel_F')
    plt.plot(epochs, evr_scores, 'm-', linewidth=2, label='EVR')
    plt.xlabel('Epoch')
    plt.ylabel('Score')
...
def main():
    ...
    # AdaLoRA/RankAllocator 相关可选参数
    parser.add_argument('--use_rank_allocator', action='store_true', help='Enable dynamic rank allocator for AdaLoRA')
    parser.add_argument('--allocator_init_warmup', type=int, default=1000)
    parser.add_argument('--allocator_final_warmup', type=int, default=1000)
    parser.add_argument('--allocator_mask_interval', type=int, default=20)
    parser.add_argument('--allocator_keep_ratio', type=float, default=None)
    parser.add_argument('--lambda_act', type=float, default=0.0, help='Activation orthogonality regularization weight')
    parser.add_argument('--arch', type=str, default='ResMLP', choices=['ResMLP', 'MLP', 'AdaLoRA'], help='Model architecture')
    parser.add_argument('--resume', action='store_true', help='Resume training from best checkpoint in arch dir')
    
    args = parser.parse_args()
    
    # 生成时间戳用于日志命名
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    log_dir = f"log/logs_{timestamp}"
    ...
    # 结果目录按架构分类
    arch_name = args.arch
    results_dir = os.path.join(args.results_dir, arch_name)
    os.makedirs(results_dir, exist_ok=True)

    # 打印配置信息并保存到日志
    config_info = f"""训练配置信息:
    图像尺寸: 448x448 (16x16 patches)
    Token数量: {T}
    特征维度: {D}
    秩: {r}
    批次大小: {batch_size}
    ...
    try:
        ...
        # Fallback: 使用模拟数据（内联生成，避免外部依赖）
        def _generate_dummy_dataset(num_samples: int, tokens: int, dim: int):
            data = torch.randn(num_samples, tokens, dim)
            class _DS(torch.utils.data.Dataset):
                ...
                def __getitem__(self, idx):
                    return data[idx]
            return _DS()
        dataset = _generate_dummy_dataset(2000, T, D)
    ...
    # 创建模型
    model = get_model_by_name(arch_name, input_dim=D, num_tokens=T, rank=r, normalize=False)
    model.to(device)
    print(f"✅ Model created and moved to {device}")

    # 创建优化器和学习率调度器
    optimizer = torch.optim.AdamW(model.parameters(), lr=learning_rate, weight_decay=weight_decay)
    # 使用主流动态调度器：ReduceLROnPlateau 已满足按验证指标动态调整
    ...
    if args.resume and os.path.exists(best_path):
        ...
        if "scheduler_state" in checkpoint:
            try:
                ...
                print(f"⚠️  scheduler 状态恢复失败：{e}")
        if "history" in checkpoint:
            history = checkpoint["history"]
        if "best_val_mse" in checkpoint:
            best_val = checkpoint["best_val_mse"]
        if "epoch" in checkpoint:
            start_epoch = int(checkpoint["epoch"]) + 1
            best_epoch = int(checkpoint.get("best_epoch", checkpoint["epoch"]))
        print(f"➡️  从第 {start_epoch} 个epoch继续，当前best MSE={best_val:.6f}")

    # 构建 RankAllocator（仅当开启且模型包含 lora_E 时有效）
    allocator = None
    if args.use_rank_allocator and hasattr(model, 'lora_E'):
        allocator = RankAllocatorWithUpdate(
            model=model,
            optimizer=optimizer,
            init_warmup=args.allocator_init_warmup,
            final_warmup=args.allocator_final_warmup,
            mask_interval=args.allocator_mask_interval,
            keep_ratio=args.allocator_keep_ratio,
        )

    print(f"\nStart training with early stopping (patience: {args.patience})...")
    # 若resume，则继续训练额外的epochs；否则从1开始
    end_epoch = start_epoch + num_epochs - 1
    for epoch in range(start_epoch, end_epoch + 1):
        t0 = time.time()
        train_stats = train_one_epoch(model, train_loader, optimizer, device, lambda_B, lambda_ortho, args.lambda_act)
        val_stats = evaluate(model, val_loader, device)
        # allocator 更新（按步更细；这里按 epoch 末尾保证最低侵入）
        if allocator is not None:
            try:
                step = epoch
                allocator.update_and_mask(step)
            except Exception as e:
                print(f"⚠️  RankAllocator 更新失败：{e}")
        ...
        history.append(log)
        epoch_log = f"Epoch {epoch:03d} | train_loss={log['train_loss']:.6f} train_mse={log['train_mse']:.6f} " \
                   f"val_mse={log['val_mse']:.6f} rel_f={log['val_rel_f']:.6f} evr={log['val_evr']:.6f} " \
                   f"lr={log['lr']:.2e} time={epoch_time:.1f}s"
        print(epoch_log)
        
        # 记录训练日志
        with open(os.path.join(log_dir, "training.log"), "a", encoding="utf-8") as f:
            f.write(epoch_log + "\n")
        ...
        if val_stats["mse"] < best_val:
            ...
            print(f"✅ New best checkpoint saved at epoch {epoch}: best_model.pt (Val MSE: {best_val:.6f})")
        else:
            patience_counter += 1
    ...
    # 保存最终结果到日志
    with open(os.path.join(log_dir, "final_results.txt"), "w", encoding="utf-8") as f:
        f.write(final_log)
        f.write(f"\n最佳epoch: {best_epoch}\n")
        f.write(f"训练配置: rank={r}, lr={learning_rate}, batch_size={batch_size}\n")
